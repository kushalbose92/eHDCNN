...
class PositionalEncoding(nn.Module):
    ...
    def map_euc_feat_to_hyp(self, x):
        x_hyp = self.manifold.proj(self.manifold.expmap0(self.manifold.proj_tan0(x, self.c), c=self.c), c=self.c)
        return x_hyp
    
    # mapping from Hyperbolic space to Euclidean space
    def map_hyp_feat_to_euc(self, x_hyp):
        x = self.manifold.proj_tan0(self.manifold.logmap0(x_hyp, c=self.c), c = self.c)
        return x
    
    # Mobius addition in hyperbolic space
    def mobius_add_with_pe(self, x, y, c):
        # h_hyp = self.manifold.mobius_add(h_hyp, p, c)
