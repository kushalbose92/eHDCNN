...
def train(model, optimizer, x_train, y_train):
    ...
    for i in range(s):
        out = model(x_train[i])
        loss_val= F.mse_loss(out, y_train[i])
        total_loss+=loss_val
    total_loss.backward()
    optimizer.step()
...
data_df=pd.DataFrame(store, columns=['Number of Epochs', 'Test RMSE Loss'])
data_df.to_csv(r"/home/mlrl/Sagar_Ghosh_ML_Lab/HDCNN/Real_Dataset/WISDM/f2_test_rms_10000.csv")
